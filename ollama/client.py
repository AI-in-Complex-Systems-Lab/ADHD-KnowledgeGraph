...
def chat(model_name, messages, tools=None, options=None, callback=None):
    try:
        ...
        with requests.post(url, json=payload, stream=True) as response:
            ...
            # Creating a variable to hold the final response
            final_response = None
            ...
            for line in response.iter_lines():
                if line:
                    ...
                    if callback:
                        ...
                        # If this is not the last chunk, add the "message" field value to full_response and print it
                        if not chunk.get("done"):
                            message_piece = chunk.get("message", {}).get("content", "")
                            full_response += message_piece
                            print(message_piece, end="", flush=True)
                    
                    # Check if it's the last chunk (done is true)
                    if chunk.get("done"):
                        final_response = chunk
            
            # Return the full response and the final response
